...
def init_setup():
    ...
    cumulants_vec = cumulant_fix(cumulants_vec)
    cum_dx_vec = cumulant_fix(cum_dx_vec)
    cum_real_vec = cumulant_fix(cum_real_vec)
    cum_imag_vec = cumulant_fix(cum_imag_vec)
    # convert to array
    print('convert data')
    ...
    sampels_real = np.array(cum_real_vec)
    samples_imag = np.array(cum_imag_vec)
    
    return samples, samples_dx, sampels_real, samples_imag, labels, snr
...
def import_data(filepath):
    with h5py.File(filepath, 'r') as f:
        ...
        snr = f['Z']
        cumulants_vec, cum_dx_vec, cum_real_vec, cum_imag_vec, labels, snr_vec = process_data(data,modulation_onehot,snr)
...
def process_data(data,modulation_onehot,snr):
    mods_total = ['OOK','4ASK','8ASK','BPSK','QPSK','8PSK','16PSK','32PSK','16APSK','32APSK','64APSK','128APSK','16QAM','32QAM',
    '64QAM','128QAM','256QAM','AM-SSB-WC','AM-SSB-SC','AM-DSB-WC','AM-DSB-SC','FM','GMSK','OQPSK'] # all labels
    ...
    for idx in range(snr.size):
        ...
        #     continue
        if idx % 10000 == 0:
            print('we have reached ',idx, ' current modulation is ',current_modulation,' SNR=',snr[idx][0],'dB')
        signal = data[idx]
        real = signal[:,0]
        imag = signal[:,1]
...
def cumulant_generation_real(signal):   
    ...
    # get the cumulants
    cumulant = np.empty(3)
    cumulant[0] = M_40 - 3*pow(M_20,2) # c_40
    cumulant[1] = M_60 -15*M_40*M_20 + 30*pow(M_20,3) # c_60
    cumulant[2] = M_80 - 28*M_60*M_20 - 35*pow(M_40,2) + 420*M_40*pow(M_20,2) - 630*pow(M_20,4) # c_80
...
def cumulant_generation_complex(input):   
    ...
    # get the cumulants
    cumulant = np.empty(6, dtype=np.complex128)
    cumulant[0] = M_40 - 3*pow(M_20,2) # c_40
    cumulant[1] = M_41 - 3*M_21*M_20 # c_41
    cumulant[2] = M_42 - pow(abs(M_20),2) - 2*pow(M_21,2) # c_42
    cumulant[3] = M_60 -15*M_40*M_20 + 30*pow(M_20,3) # c_60
    cumulant[4] = M_63 - 9*cumulant[2]*M_21 - 6*pow(M_21,3) # c_63
    cumulant[5] = M_80 - 28*M_60*M_20 - 35*pow(M_40,2) + 420*M_40*pow(M_20,2) - 630*pow(M_20,4) # c_80
...
def cumulant_fix(cumulants):
    ...
    for ii in range(len(cumulants)):
        ...
        # feats.append((cum_abs[2]/(cum_abs[1])))
        # feats.append((cum_abs[2]/(cum_abs[0])))
        # feats.append((cum_abs[4]/(cum_abs[3])))
        # feats.append((pow(cum_abs[4],2)/pow(cum_abs[0],3)))
        # feats.append((cum_abs[5]/pow(cum_abs[0],2)))
        # feats.append((cum_abs[5]/pow(cum_abs[1],2)))
